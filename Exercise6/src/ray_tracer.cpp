...
glm::vec3 ray_tracer::ray_view_dir(const glm::ivec2 & pos, const glm::ivec2 & window_size, const glm::mat4 & inv_proj_view, const glm::vec3 & cam_pos)
{
	...
	ndc.y = 1.0f - (2.0f * pos.y) / window_size.y;
...
float rt_simple::intersect_depth(const glm::vec3 & org, const glm::vec3 & dir){
	...
	for(auto face:faces){
		...
		float d = -n * p1;
		if(fabsf(normal*dir)<EPSILON) continue;
		
		float t = (-d - normal*org )/(normal * dir);
		...
		if (cross(p1-p0, p_intersect-p0) * normal < 0 ) continue;
  		if (cross(p2-p1, p_intersect-p1) * normal < 0 ) continue;
  		if (cross(p0-p2,p_intersect-p2) * normal < 0 ) continue;
