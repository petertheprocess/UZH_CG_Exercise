...
namespace cgCourse
{
	...
	void GLApp::keyPressHandler(int _key, int _action, int _mods)
	{
		if(_key == GLFW_KEY_Q && _action == GLFW_PRESS)
		{
			...
			if(settings.action){
				pauseTime = glfwGetTime()-pauseTime;
				timeOffset += pauseTime;
			}
			else
				pauseTime = glfwGetTime();
			printf("settings.action:%d\n",settings.action);
		}else if(_key == GLFW_KEY_N && _action == GLFW_PRESS){
			settings.normalMatrixOn = !settings.normalMatrixOn;
			printf("settings.normalMatrixOn:%d\n",settings.normalMatrixOn);
		}
	}
