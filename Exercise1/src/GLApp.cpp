...
namespace cgCourse
{
	...
	{
		if ((_key == GLFW_KEY_ESCAPE || _key == GLFW_KEY_Q) && _action == GLFW_PRESS)
		{
			stop();
		}
		...
		else if (_key == GLFW_KEY_C && _action == GLFW_PRESS){
			settings.cull = !settings.cull;
			printf("cull:%d\n", settings.cull);
		}
		else if (_key == GLFW_KEY_LEFT_BRACKET && _action == GLFW_PRESS){
			offset-=0.05f;
			if(offset<-2){
				offset = -2;
			}
			printf("offset:%f\n",offset);
		}else if (_key == GLFW_KEY_RIGHT_BRACKET && _action == GLFW_PRESS){
			offset+=0.05f;
			if(offset>5){
				offset = 5;
			}
	...
	bool GLApp::checkGlError(const char *_message) const
	{
		...
		if (error != GL_NO_ERROR)
		{
			...
			std::cerr << "[" << _message << "] OpenGL Error("
